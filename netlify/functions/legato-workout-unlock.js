// =============================================================================
// Netlify function: the gate for /legato-workout
//
...
//   LEGATO_WORKOUT_PRODUCT_ID   the Lemon Squeezy product id for this product.
...
const { buildRoutine, ROUTINE_META } = require("./legato-workout-routine.js");
...
function wrongProduct(meta) {
  const want = process.env.LEGATO_WORKOUT_PRODUCT_ID;
...
exports.handler = async function (event) {
  ...
  const out = {
    ...
    meta: ROUTINE_META,
    sections: buildRoutine(),
  };

  // Until LEGATO_WORKOUT_PRODUCT_ID is set there is nothing to check the key
  ...
  if (!process.env.LEGATO_WORKOUT_PRODUCT_ID) {
    const m = d.meta || {};
    out.unconfigured = {
      note: "Set LEGATO_WORKOUT_PRODUCT_ID to this productId. Any key from this store opens the app until you do.",
