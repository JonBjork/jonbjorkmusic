...
export default function NpsWorkout({onBack,legato=false,sweep=false,five=false}){
 ...
 <div className="prs-eyebrow">THE PRACTICE ROOM CHOPS BUILDERS</div><h1>{title}</h1>
 ...
 {!sweep&&<details style={{marginBottom:24}}><summary>Watch the introduction</summary><iframe title={`${title} introduction`} src={`https://www.youtube-nocookie.com/embed/${legato?'YN0IZGywlBs':'VuziMUlW6tQ'}`} loading="lazy" allowFullScreen style={{width:'100%',maxWidth:850,aspectRatio:'16/9',border:0}}/><p>80 BPM · eighth notes is the starting suggestion. Repeat the routine over several days and try different keys. {legato?'Pick when changing strings, then use hammer-ons and pull-offs. Keep the dynamics even.':'Every position reverses the starting pick stroke.'}</p></details>}
 ...
 </section><aside>{sweep&&<><h2>This exercise</h2><strong className="prs-duration">{fmtClock(done?0:Math.max(0,passage.duration-cursor*60/bpm/subdivision))}</strong></>}<h2>Time remaining</h2><strong className="prs-duration">{fmtClock(done?0:(sweep?data.passages.slice(pass).reduce((sum,p)=>sum+p.duration,0)-cursor*60/bpm/subdivision:remaining*60/bpm/subdivision)+(Math.max(0,data.passages.length-pass-1)*2+(countTotal===2?(count||0):0))*60/bpm)}</strong><p>Includes two beats between sequences. Initial count-in is extra.</p>
