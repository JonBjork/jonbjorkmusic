...
import { subdivisionOffsets } from './subdivisionTiming';
// ─────────────────────────────────────────────────────────────────────────────
...
const CLICK_URL = "metronome-sounds/click.wav";
const HIHAT_URL = "metronome-sounds/hihat.wav";
...
export function createMetronomeEngine() {
  ...
  function loop() {
    ...
    while (state.nextNoteTime < horizon && state.running) {
      ...
      const aud = Math.max(1, state.audibleSubdivision || sub);
      if (!state.muted || isCountIn) {
        if (isMainBeat) {
        ...
        if (!isCountIn) {
          for (const offset of subdivisionOffsets(state.subdivPosition, sub, aud)) {
            playClick(hihatBuffer, state.nextNoteTime + offset * 60 / state.bpm, 0.6);
          }
        }
      }
