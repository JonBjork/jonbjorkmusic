import {buildGroups,cycleLayout,stages,stageLabel,localDay,freshChunkState,validateChunkState,mergeChunkState} from './chunk-model.js';
...
export function mountChunkPlayer(root,config) {
  ...
  let running=false,loading=false,token=0,queue=[],frame=0,finishAt=null,entry=null,lastSave=0,activeNote=-1,unlock=null;
  ...
  function credit(seconds,repetition=false,completion=false){
    ...
    entry.seconds+=seconds;entry.repetitions+=Number(repetition);entry.completions+=Number(completion);progress().elapsed+=seconds;
  }
  function reconcile(){const now=getAudioContext()?.currentTime??0;
    for(const q of queue){if(now<q.when)break;const amount=Math.max(0,Math.min(now,q.end)-q.when-q.credited);if(amount){credit(amount);q.credited+=amount;}if(now>=q.end&&!q.finished){q.finished=true;if(q.last){const done=progress().elapsed+1e-6>=state.settings.timerSeconds;credit(0,true,done);if(done)progress().done=true;}}}
    queue=queue.filter(q=>!q.finished);timer();
  }
  function stop(text='Paused. Resume continues your timer from the start of this group.'){
    if(running)reconcile();token++;running=false;loading=false;engine.stop();stopGuitar();cancelAnimationFrame(frame);queue=[];finishAt=null;entry=null;highlight(-1);if(unlock){unlock();unlock=null;}save();controls();$('chunkStatus').textContent=text;
  }
  ...
  async function start(){
    ...
    if(progress().done){progress().elapsed=0;progress().done=false;}
    ...
    try{
      ...
      const g=group(),layout=cycleLayout(g),seconds=60/state.settings.bpm/3,baseElapsed=progress().elapsed;
      let scheduledSeconds=0,stopScheduling=false;
      ...
      const ok=await engine.start({bpm:state.settings.bpm,subdivision:3,countInBeats:0,audibleSubdivision:state.settings.subdivisions?3:1,
        onScheduleTick(i,when){
          if(my!==token||stopScheduling)return false;
          ...
          if(inMusic){
            ...
            queue.push({when,end:when+seconds,credited:0,finished:false,last});scheduledSeconds+=seconds;
            if(at===note.at&&state.settings.instrumentOn)pluck(getAudioContext(),note.midi,when,note.len*seconds,.5,{tight:note.len===1});
            if(last&&baseElapsed+scheduledSeconds+1e-6>=state.settings.timerSeconds){finishAt=when+seconds;stopScheduling=true;}
          }
          return true;
        },
        onTick(i){if(my!==token)return;const pos=i%layout.length,at=pos-layout.musicStart;
          if(pos<layout.musicStart){highlight(-1);$('chunkStatus').textContent=pos<12?`Count in · ${4-Math.floor(pos/3)}`:'Pickup next…';}
          else if(pos<layout.musicEnd){highlight(g.notes.findIndex(n=>at>=n.at&&at<n.at+n.len));$('chunkStatus').textContent=progress().elapsed>=state.settings.timerSeconds?'Finishing this repetition…':'Playing';}
