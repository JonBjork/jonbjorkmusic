...
//     onScheduleTick(idx, when), // like onTick, but fires synchronously when
//                            // the tick is SCHEDULED (up to ~100ms before it
//                            // sounds); `when` is its AudioContext time, so
//                            // a caller can put its own sound on the same
//                            // clock (Workouts plays the tab through it).
//                            // setSubdivision/setMuted calls made here apply
//                            // to that tick and its interval — use it for
//                            // boundary-exact changes. Return false to halt
//                            // BEFORE this tick: nothing more is scheduled,
//                            // but every tick already scheduled still sounds
//                            // and still reports through onTick, so a run
//                            // can end cleanly on a chosen note.
//     audibleSubdivision,    // optional: how many ticks per beat SOUND when
//                            // that is fewer than `subdivision` (1 = clicks
//                            // on the beat only while still ticking every
//                            // subdivision). Change live with
//                            // setAudibleSubdivision(n).
//   });
//   engine.setBpm(120); engine.setSubdivision(2);
//   getAudioContext()        // the shared context, for scheduling other
//                            // sounds on the metronome's clock. Null until
//                            // audio has been initialised.
//   engine.stop();
//
...
const SOUND_BASE =
  (typeof window !== "undefined" && window.__PL_SOUND_BASE) || new URL("./audio/metronome", import.meta.url).href;
...
function playSample(buffer, time, volume = 1, output = audioCtx?.destination) {
...
export function createMetronomeEngine() {
  let volume = 1, volumeBus = null, volumeContext = null;
  function outputBus() {
    if(volumeContext !== audioCtx) {
      volumeBus=audioCtx.createGain();volumeBus.gain.value=volume;
      volumeBus.connect(audioCtx.destination);volumeContext=audioCtx;
    }
    return volumeBus;
  }
  function setVolume(value) {
    volume=Number.isFinite(value)?Math.max(0,Math.min(1,value)):1;
    if(volumeBus && volumeContext===audioCtx)volumeBus.gain.setTargetAtTime(volume,audioCtx.currentTime,0.015);
  }
  function playClick(buffer,time,level) {
    if(audioCtx)playSample(buffer,time,level,outputBus());
  }
  ...
  const state = {
    ...
    audibleSubdivision: null, // ticks per beat that sound; null = all of them
  ...
  function intervalSec() {
    if (state.countInRemaining > 0) {
      return 60 / state.bpm / (state.countInSubdivide ? Math.max(1, state.countInSubdivision || state.subdivision) : 1);
    }
  ...
  function loop() {
    ...
    while (state.nextNoteTime < horizon && state.running) {
      ...
      if (!isCountIn && state.onScheduleTick) {
        let go = true;
        try { go = state.onScheduleTick(state.tickIndex, state.nextNoteTime); } catch (e) {}
        if (go === false) { haltScheduling(); return; }
      }
      ...
      const countInSub = state.countInSubdivide ? Math.max(1,state.countInSubdivision || sub) : 1;
      ...
      // audibleSubdivision thins the sound without changing the tick rate:
      // with 16th ticks and audibleSubdivision 2, only every other tick
      // sounds, so the player hears eighths while the tab still advances
      // every 16th.
      // A subdividing count-in is not thinned: hearing every subdivision
      // before the first note is what it is for.
      const aud = Math.max(1, Math.min(sub, state.audibleSubdivision || sub));
      const stride = Math.max(1, Math.round(sub / aud));
      const audible = (!state.muted || isCountIn) &&
        (isCountIn || isMainBeat || state.subdivPosition % stride === 0);
      if (audible) {
        if (isMainBeat) {
          playClick(clickBuffer, state.nextNoteTime, 0.95);
          if ((isCountIn ? countInSub : aud) > 1) playClick(hihatBuffer, state.nextNoteTime, 0.6);
        } else {
          playClick(hihatBuffer, state.nextNoteTime, 0.6);
        }
      }
  ...
  async function start({ bpm, subdivision, countInBeats = 4, countInSubdivide = false, countInSubdivision, onTick, onCountIn, onScheduleTick, audibleSubdivision } = {}) {
    ...
    // stop() during the wait above clears `starting`: the caller has moved
    // on, so this start must not bring the scheduler up behind its back.
    // (On a phone the wait can run past a second, long enough to tap Pause.)
    if (!ok || !state.starting) { state.starting = false; return false; }
    if (typeof bpm === "number") state.bpm = bpm;
    if (typeof subdivision === "number") state.subdivision = subdivision;
    state.audibleSubdivision = typeof audibleSubdivision === "number" ? audibleSubdivision : null;
    ...
    state.countInSubdivision = Number.isFinite(countInSubdivision) ? countInSubdivision : null;
  ...
  // Stop scheduling but let what is already in the lookahead window play out
  // and report. Used when onScheduleTick returns false; stop() is the hard
  // version that also cancels the pending callbacks.
  function haltScheduling() {
    state.running = false;
    state.starting = false;
    releaseWakeLock();
  ...
  function setAudibleSubdivision(n) {
    state.audibleSubdivision = typeof n === "number" ? Math.max(1, Math.round(n)) : null;
  }
  ...
  function patternLoop() {
    ...
    while (patternState.running && patternState.nextNoteTime < horizon) {
      ...
      if (patternState.countInRemaining > 0) {
        playClick(clickBuffer, patternState.nextNoteTime, 0.95);
      ...
      if (isBeat) {
        ...
        playClick(hihatBuffer, patternState.nextNoteTime, 0.7);
      }
